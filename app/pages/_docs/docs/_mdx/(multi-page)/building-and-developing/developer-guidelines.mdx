...
<Callout
  type="info"
  title="Excluding Non-Public Interfaces from API Documentation"
>
...
<Callout type="warning">
  Replication may be disrupted and data loss may occur if you delete the
  replication tree (_/hbase/replication/_) from ZooKeeper. Follow progress on
  this issue at
...
<Callout type="warn" title="Before you commit!!!!">
  ...
  email. Examine the output of the `$ git config --list` command and be sure it
  is correct. See [Set Up Git](https://help.github.com/articles/set-up-git) if
  you need pointers.
</Callout>
...
2. Commit the patch to a new branch based off `master` or the other intended
   ...
   <Callout type="warning">
     If you do not have all remote changes, the push will fail. If the push
     fails for any reason, fix the problem or ask for help. Do not do a `git
     push --force`.
   </Callout>
