...
<Callout type="info">
  The _hbase_ script assumes all integration tests with exposed _main_ methods
  to be run against a distributed cluster will follow the **IntegrationTest**
  regex naming pattern mentioned above, in order to proper set test dependencies
  into the classpath.
</Callout>
