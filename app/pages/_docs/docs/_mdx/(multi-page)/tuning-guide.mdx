...
- For write-heavy data:
  ...
  - Create local indexes. Reads from local indexes have a performance penalty, so it's important to do performance testing. See the [Pherf](/docs/performance-testing) tool.
...
- Use the `ASYNC` keyword with your `CREATE INDEX` call to create the index asynchronously via MapReduce job. You'll need to manually start the job; see [Index Population](/docs/secondary-indexes#index-population) for details.
...
- If you need transactionality, use the `TRANSACTIONAL` [option](/docs/grammar#options). (See also [Transactions](/docs/transactions))
...
See also:
[Secondary Indexing](/docs/secondary-indexes)

## Secondary indexes [#tuning-guide-secondary-indexes]

Secondary indexes can improve read performance by turning what would normally be a full table scan into a point lookup (at the cost of storage space and write speed). Secondary indexes can be added or removed after table creation and don't require changes to existing queries – queries simply run faster. A small number of secondary indexes is often sufficient. Depending on your needs, consider creating _[covered](/docs/secondary-indexes#covered-indexes)_ indexes or _[functional](/docs/secondary-indexes#functional-indexes)_ indexes, or both.
...
See [Index Population](/docs/secondary-indexes#index-population) for details.
...
- Create [local](/docs/secondary-indexes#local-indexes) indexes for write-heavy use cases.
- Create global indexes for read-heavy use cases. To save read-time overhead, consider creating [covered](/docs/secondary-indexes#covered-indexes) indexes.
...
You can improve parallelization with the [UPDATE STATISTICS](/docs/statistics-collection) command. This command subdivides each region by determining keys called _guideposts_ that are equidistant from each other, then uses these guideposts to break up queries into multiple parallel scans.
