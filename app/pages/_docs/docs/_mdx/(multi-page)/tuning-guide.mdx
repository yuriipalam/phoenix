...
Secondary indexes can improve read performance by turning what would normally be a full table scan into a point lookup (at the cost of storage space and write speed). Secondary indexes can be added or removed after table creation and don't require changes to existing queries – queries simply run faster. A small number of secondary indexes is often sufficient. Depending on your needs, consider creating _[covered](/docs/secondary-indexing#Covered_Indexes)_ indexes or _[functional](/docs/secondary-indexing#Functional_Indexes)_ indexes, or both.
...
See also: [Joins](https://phoenix.apache.org/joins.html)
...
See also: [Hint](/docs/language/index#hint).

### Explain plans

An `EXPLAIN` plan tells you a lot about how a query will be run. To generate an explain plan run [this](/docs/language/index#explain) query and to interpret the plan, see [this](/docs/explain-plan) reference.

### Parallelization

You can improve parallelization with the [UPDATE STATISTICS](/docs/update-statistics) command. This command subdivides each region by determining keys called _guideposts_ that are equidistant from each other, then uses these guideposts to break up queries into multiple parallel scans.
...
To reduce RPC traffic, set `UPDATE_CACHE_FREQUENCY` (4.7 or above) on your tables and indexes when creating them (or via `ALTER TABLE`/`ALTER INDEX`). See [Altering](https://phoenix.apache.org/#Altering).
