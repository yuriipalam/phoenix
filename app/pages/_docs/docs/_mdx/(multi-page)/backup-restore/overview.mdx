...
<Callout type="warn">
  ...
  Encryption (TDE) enabled HDFS clusters. This is related to the open issue
...
<Callout type="info">
  ...
  workaround is to manually create copies of the backup files from HDFS or S3.
</Callout>
...
<Callout type="info">
  The ",..." is an ellipsis meant to imply that this is a comma-separated list
  of values, not literal text which should be added to hbase-site.xml.
</Callout>
