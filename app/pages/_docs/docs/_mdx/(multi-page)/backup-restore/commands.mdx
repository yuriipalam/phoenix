...
<Callout type="info">
  ...
  tables in the backup. In the event that you need to restore the HBase backup,
  access to the system catalog tables enable you to resume Phoenix
  interoperability with the restored data.
</Callout>
...
<Callout type="info">
  ...
  the source cluster fails and you need to recover the dataset with a restore
...
<Callout type="info">
  If the namespace of a table being restored does not exist in the target
  environment, it will be automatically created during the restore operation.
...
<Callout type="info">
  ...
  _-set_ option. The `hbase backup set add` command must be run before using the
  `-set` option in a different command because backup sets must be named and
  defined before using backup sets as a shortcut.
</Callout>
...
<Callout type="info">
  ...
  to the original HBase cluster with the backup set metadata, then you must
  specify individual table names to restore the data.
</Callout>
...
<Callout type="info">
  ...
  hbase backup set to complete an operation. Also, the backup set name is
  case-sensitive in the command-line utility.
</Callout>
...
<Callout type="info">
  ...
  corresponding tables in each set on a separate or remote cluster, backup
  strategy. This information can help you in case of failure on the primary
  cluster.
</Callout>
