...
<Callout type="info">
  The stateless endpoints are optimized for small results, while the [stateful
  scanner API](/docs/external-apis#stateful-endpoints-for-scan-operations) can
  also be used for large results.
</Callout>
...
- In the above json-format example:
  ...
  5.  The `"row key" in the request URL` should be URL-encoded, e.g., `"david%20chen"` and `"row1"` are the URL-encoded formats of row keys `"david chen"` and `"row1"`, respectively
      <Callout type="info">
        Note: "cfa" is the column family name and "alias" are the column
