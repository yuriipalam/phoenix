...
To disable column mapping across all new tables, you need to set `phoenix.default.column.encoded.bytes.attrib` to 0. One can also leave it on globally and have it disabled selectively for a table by setting the COLUMN_ENCODED_BYTES = 0 property in the create table statement.
