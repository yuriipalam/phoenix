...
Note that if the primary table is salted, then the index is automatically salted in the same way for global indexes. In addition, the MAX_FILESIZE for the index is adjusted down, relative to the size of the primary versus index table. For more on salting see [here](/docs/salted-tables). With local indexes, on the other hand, specifying `SALT_BUCKETS` is not allowed.
...
See [resources](/docs/secondary-indexes#secondary-indexes-resources) for more in-depth information.

All newly created tables use the new indexing algorithm.

Indexes created with older Phoenix versions will continue to use the old implementation, until upgraded with [IndexUpgradeTool](/docs/secondary-indexes#index-upgrade-tool)
