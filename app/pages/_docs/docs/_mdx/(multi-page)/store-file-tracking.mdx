...
<Callout type="info">
  When the store file tracking implementation is specified in
  **hbase_site.xml**, this configuration is also propagated into a tables
  configuration at table creation time. This is to avoid dangerous configuration
  mismatches between processes, which could potentially lead to data loss.
</Callout>
...
<Callout type="info">
  MIGRATION tracker should NOT be set at global configuration. To use it, follow
  below section about setting Store File Tacking at Table or Column Family
  configuration.
</Callout>
...
<Callout type="info">
  The option to specify the tracker during snapshot recovery is only available
  for the _clone_snapshot_ command. The _restore_snapshot_ command does not
