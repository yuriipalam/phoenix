...
are equal byte distanced from each other. These collected keys are called _guideposts_
and they act as _hints/guides_ to improve the parallelization of queries on a given
target region.
