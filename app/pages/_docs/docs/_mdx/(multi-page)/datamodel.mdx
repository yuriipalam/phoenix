...
<Callout type="info" title="Column Names">
  ...
  _qualifier_. For example, the column _contents:html_ is made up of the column
  family `contents` and the `html` qualifier. The colon character (`:`) delimits
  the column family from the column family _qualifier_.
</Callout>
...
<Callout type="info">
  ...
  HBase 0.94, and was fixed in
  [HBASE-10118](https://issues.apache.org/jira/browse/HBASE-10118). The change
