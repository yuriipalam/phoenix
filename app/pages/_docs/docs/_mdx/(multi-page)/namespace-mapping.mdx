...
| --- | --- | --- |
| `phoenix.schema.isNamespaceMappingEnabled` | If it is enabled, then the tables created with schema will be mapped to namespace. This needs to be set at client and server both. If set once, it should not be rolled back. Old client will not work after this property is enabled. | `false` |
| `phoenix.schema.mapSystemTablesToNamespace` | Enabling this property will take affect when `phoenix.connection.isNamespaceMappingEnabled` is also set to true. If it is enabled, SYSTEM tables if present will automatically migrated to SYSTEM namespace. If set to false, then system tables will be created in default namespace only. This needs to be set at client and server both. | `true` |
...
###  How system tables will be migrated?
...
###  How schemas are mapped for different table types?
