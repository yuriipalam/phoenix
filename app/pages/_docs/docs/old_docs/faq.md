...
*Pre-requisite:* [Download](/docs/download) and [install](/docs/installation) the latest Phoenix.
...
Upsert rows in this test table and Phoenix query optimizer will choose correct index to use. You can see in [explain plan](/docs/language/index#explain) if Phoenix is using the index table. You can also give a [hint](/docs/language/index#hint) in Phoenix query to use a specific index.
...
You can force Phoenix to use secondary for uncovered columns by specifying an [index hint](/docs/index#index_hint)
