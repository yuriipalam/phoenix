...
* For write-heavy data:
    ...
    * Create local indexes. Reads from local indexes have a performance penalty, so it's important to do performance testing. See the [Pherf](/docs/pherf) tool.
...
* If the data is immutable or append-only, declare the table and its indexes as immutable using the `IMMUTABLE_ROWS` [option](/docs/language/index#options) at creation time to reduce the write-time cost. If you need to make an existing table immutable, you can do so with `ALTER TABLE trans.event SET IMMUTABLE_ROWS=true` after creation time.
    * If speed is more important than data integrity, you can use the `DISABLE_WAL` [option](/docs/language/index#options). Note: it is possible to lose data with `DISABLE_WAL` if a region server fails. 
* Set the `UPDATE_CACHE_FREQUENCY` [option](/docs/language/index#options) to 15 minutes or so if your metadata doesn't change very often. This property determines how often an RPC is done to ensure you're seeing the latest schema.
...
* If the data is too large to scan the table completely, use primary keys to create an underlying composite row key that makes it easy to return a subset of the data or facilitates [skip-scanning](/docs/skip-scan)—Phoenix can jump directly to matching keys when the query includes key sets in the predicate.
...
* If you need transactionality, use the `TRANSACTIONAL` [option](/docs/language/index#options). (See also http://phoenix.apache.org/transactions.html.)
...
See also: 
[Secondary Indexing](/docs/secondary-indexing)

## Secondary indexes

Secondary indexes can improve read performance by turning what would normally be a full table scan into a point lookup (at the cost of storage space and write speed). Secondary indexes can be added or removed after table creation and don't require changes to existing queries – queries simply run faster. A small number of secondary indexes is often sufficient. Depending on your needs, consider creating *[covered](/docs/secondary-indexing#Covered_Indexes)* indexes or *[functional](/docs/secondary-indexing#Functional_Indexes)* indexes, or both.
...
See [Index Population](/docs/secondary-indexing#Index_Population) for details.
...
* Create [local](/docs/secondary-indexing#Local_Indexes) indexes for write-heavy use cases.
* Create global indexes for read-heavy use cases. To save read-time overhead, consider creating [covered](/docs/secondary-indexing#Covered_Indexes) indexes.
...
An `EXPLAIN` plan tells you a lot about how a query will be run. To generate an explain plan run [this](/docs/language/index#explain) query and to interpret the plan, see [this](/docs/explainplan) reference.

### Parallelization

You can improve parallelization with the [UPDATE STATISTICS](/docs/update-statistics) command. This command subdivides each region by determining keys called *guideposts* that are equidistant from each other, then uses these guideposts to break up queries into multiple parallel scans.
